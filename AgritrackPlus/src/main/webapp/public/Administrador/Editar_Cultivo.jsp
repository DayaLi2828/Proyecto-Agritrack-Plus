...
<%
    ...
    if (id == null || id.isEmpty()) {
        response.sendRedirect("Cultivos_Registrados.jsp");
        return;
    }
    ...
    List<String> idsAsignados = new ArrayList<>();
    if (trabajadoresAsignados != null) {
        for (Map<String, String> t : trabajadoresAsignados) {
            idsAsignados.add(String.valueOf(t.get("id"))); 
        }
    }

    // --- Lógica de Productos (Insumos) ---
    ProductoDAO productoDAO = new ProductoDAO();
    List<Map<String, String>> todosLosProductos = productoDAO.listarProductos(); 
    List<Map<String, String>> productosAsignados = dao.obtenerProductosCultivo(Integer.parseInt(id));
    
    List<String> idsProductosAsignados = new ArrayList<>();
    if (productosAsignados != null) {
        for (Map<String, String> p : productosAsignados) {
            idsProductosAsignados.add(String.valueOf(p.get("id")));
        }
    }
%>
<!DOCTYPE html>
    ...
    <main class="main">
        <div class="contendor">
            <% if ("true".equals(request.getParameter("error"))) { %>
                <p style="color:red; text-align:center;">Hubo un error al editar el cultivo. Intenta de nuevo.</p>
            <% } %>
            ...
            <form class="formulario__registrarcultivo" method="post" action="../../EditarCultivoServlet">
                ...
                <div class="contendor__cajas">
                    ...
                    <div class="campo">
                        <label for="fecha_cosecha">Fecha de cosecha (Opcional)</label>
                            ...
                            <li>
                                <label class="nombre__trabajador">
                                    <input type="checkbox" name="trabajadores" value="<%= trabajador.get("id") %>"
                                           <%= idsAsignados.contains(String.valueOf(trabajador.get("id"))) ? "checked" : "" %>>
                ...
                <div class="campo productos__caja">
                    <label>Insumos/Productos asignados</label>
                    <ul class="lista__productos">
                        <% if (todosLosProductos != null) {
                            for (Map<String, String> prod : todosLosProductos) { %>
                            <li>
                                <label class="nombre__producto">
                                    <input type="checkbox" name="productos" value="<%= prod.get("id") %>"
                                           <%= idsProductosAsignados.contains(String.valueOf(prod.get("id"))) ? "checked" : "" %>>
                                    <%= prod.get("nombre") %>
                                </label>
                            </li>
                        <% } } %>
